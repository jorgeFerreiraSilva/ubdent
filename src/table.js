...
function Table ({ rows }) {
  ...
  const dataRows = rows.map((item) => {
    ...
    item = {
      ...
      symtoms: (!item.anamnese.medicalData || item.anamnese.medicalData.length === 0) ? rightChoose(item.symtoms) : (item.anamnese.medicalData[item.anamnese.medicalData.length - 1]) ? rightChoose(item.anamnese.medicalData[item.anamnese.medicalData.length - 1].symtoms) : 'NDF'
